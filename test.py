...
with mp_face_mesh.FaceMesh(
    ...
    for idx in range(1,301):
        ...
        total_error = 0.
        for j in range(68):
            total_error += 100*euclaideanDistance(landmarks_extracted[j],ground_truths[j])/1.6
        total_error /= (68*interocular_distance_gt)
        # print(f"{idx}: {total_error}")
        if idx == 25 or idx == 238 or total_error> 20 :
...
plt.title('Overall NME of Indoor Images',fontsize=20)
# plt.show() 
